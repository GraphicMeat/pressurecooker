...
This is the branch-level regression gate: the full suite must be green, which means every
must-stay-green test from every task still passes against the recorded baseline. A green
feature with a broken pre-existing test is not done.
...
**If worktree path is under `.worktrees/`, `worktrees/`, or `~/.config/pressurecooker/worktrees/`:** pressureCooker created this worktree — we own cleanup.
...
- **Fix:** Always verify the full suite before offering options
...
- **Fix:** Only clean up worktrees under `.worktrees/`, `worktrees/`, or `~/.config/pressurecooker/worktrees/`
